...
# 获取网易云评论  解密需要的第一个参数
def get_comment_first_param(songid, page, next_cursor):
    ...
    if page != 1:
        key_dict['offset'] = '40'
        key_dict['cursor'] = str(next_cursor)
    return str(key_dict)
...
def get_page_information(songid, page, next_cursor):
    url = "https://music.163.com/weapi/comment/resource/comments/get?csrf_token="
    first_param = get_comment_first_param(songid, page, next_cursor)
...
def get_all_comements(songid):

    #从第一页获取总评论数
    json_dict = get_page_information(songid, 1, '')
    comment_num = json_dict['data']['totalCount']
    page_num = (comment_num // 20) + 1
    comments = []
    comments.append(get_page_comments(json_dict, 20))  # 添加第一页的数据
    for i in range(1, page_num):
        page = i+1
        next_cursor = json_dict['data']['cursor']
        json_dict = get_page_information(songid, page, next_cursor)
        page_comment_length = len(json_dict['data']['comments'])  # 每页评论的条数
        comments.append(get_page_comments(json_dict, page_comment_length))
    return comments


def get_page_comments(data,num):
    comments = []
    for i in range(num):
        content = data['data']['comments'][i]['content']
        comments.append(content)
        item = {'songid': songid, 'comment': content}
        comment_to_mongo(item, mongo_url, mongo_db)
    return comments


def comment_to_mongo(item, mongo_url, mongo_db):
    client = pymongo.MongoClient(mongo_url)
    db = client[mongo_db]
    db['comment'].insert(dict(item))
    client.close()


if __name__ == "__main__":
    get_all_comements(songid)
